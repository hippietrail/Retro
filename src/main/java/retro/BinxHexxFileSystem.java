...
/**
 * A pseudo-filesystem that contains a single file that represents the decoded
 * contents of the BinHex file.
 * <p>
 * If the filename can be recovered from the embedded metadata, it will be used as the
 * name of the singleton file, otherwise the name "binhex_decoded" will be used.
 * <p>
 * 
 */
@FileSystemInfo(type = "binhex", description = "BinHex", priority = FileSystemInfo.PRIORITY_LOW, factory = BinxHexxFileSystemFactory.class)
public class BinxHexxFileSystem implements GFileSystem {
    public static final String BINHEX_PAYLOAD_FILENAME = "binhex_decoded";
    ...
    public BinxHexxFileSystem(ByteProvider container, FSRLRoot fsFSRL, FileSystemService fsService,
            TaskMonitor monitor) throws IOException, CancelledException {
        this.fsFSRL = fsFSRL;
        this.fsService = fsService;
        this.container = container;
        
        readBinHexMetadata(monitor);
    ...
    private void readBinHexMetadata(TaskMonitor monitor) throws IOException {
        try (BinxHexxEncoderInputStream bhcis =								// apache
            new BinxHexxEncoderInputStream(container.getInputStream(0))) {	// apache
            BinxHexxParameters metaData = bhcis.getMetaData();				// apache
            payloadFilename = metaData.getFilename();
            if (payloadFilename == null) {
                String containerName = fsFSRL.getContainer().getName();
                if (containerName.toLowerCase().endsWith(".binhex")) {
                    payloadFilename = FilenameUtils.removeExtension(containerName);
                }
                else {
                    payloadFilename = BINHEX_PAYLOAD_FILENAME;
                }
            }
            else {
            ...
            this.payloadKey = "decoded " + payloadFilename;
        }
    }
    ...
    private ByteProvider getPayloadByteProvider(TaskMonitor monitor)
        ...
        return fsService.getDerivedByteProvider(container.getFSRL(), null, payloadKey, -1,
            () -> new BinxHexxEncoderInputStream(container.getInputStream(0)), upwtm);
    }
